 ...
 */
public abstract class AMPLJUnit4CacheTestCase extends JUnit4DistributedTestCase
  ...
  public AMPLJUnit4CacheTestCase() {
    this(null);
  }

  AMPLJUnit4CacheTestCase(final CacheTestFixture cacheTestFixture) {
  ...
  private final void createCache(final boolean client, final MCacheFactory factory) {
    synchronized (AMPLJUnit4CacheTestCase.class) {
      try {
        System.setProperty(
        ...
        if (client) {
          ...
          newCache = (MCache) new MClientCacheFactory(getSystem().getProperties()).create();
        } else {
          if (factory == null) {
            newCache = MCacheFactory.create(getSystem());
          } else {
   ...
   */
  public final MCache createLonerCache() {
    synchronized (AMPLJUnit4CacheTestCase.class) {
      try {
        System.setProperty(
            DistributionConfig.GEMFIRE_PREFIX + "DISABLE_DISCONNECT_DS_ON_CACHE_CLOSE", "true");
        MCache newCache = MCacheFactory.create(getLonerSystem());
        cache = newCache;
      } catch (CacheExistsException e) {
        Assert.fail("the cache already exists", e); // TODO: remove error handling
   ...
   */
  public final MCache getCache() {
    return getCache(false);
  }

  public final MCache getCache(final MCacheFactory factory) {
    return getCache(false, factory);
  }

  public final MCache getCache(final Properties properties) {
    getSystem(properties);
    return getCache();
  }

  public final MCache getCache(final boolean client) {
    return getCache(client, null);
  }

  public final MCache getCache(final boolean client, final MCacheFactory factory) {
    synchronized (AMPLJUnit4CacheTestCase.class) {
      final MonarchCacheImpl ampoolCache = MonarchCacheImpl.getInstance();
      if (ampoolCache != null && !ampoolCache.isClosed()
          && ampoolCache.getCancelCriterion().isCancelInProgress()) {
        Wait.waitForCriterion(new WaitCriterion() { // TODO: replace with Awaitility
          @Override
          public boolean done() {
            return ampoolCache.isClosed();
          }
   ...
   * @since GemFire 6.5
   */
  public final MClientCache getClientCache(final MClientCacheFactory factory) {
    synchronized (AMPLJUnit4CacheTestCase.class) {
      final MonarchCacheImpl ampoolCache = MonarchCacheImpl.getInstance();
      if (ampoolCache != null && !ampoolCache.isClosed()
          && ampoolCache.getCancelCriterion().isCancelInProgress()) {
      ...
      if (cache == null || cache.isClosed()) {
        cache = null;
        disconnectFromDS();
        cache = (MCache) factory.create();
      }
      if (cache != null) {
        IgnoredException.addIgnoredException("java.net.ConnectException");
      }
      return (MClientCache) cache;
    }
  }
   ...
   */
  public static final synchronized MCache basicGetCache() {
    return cache;
  }
  ...
  public static final synchronized void closeCache() {
    ...
    try {
      if (cache != null) {
        try {
          if (!cache.isClosed()) {
            if (cache instanceof MonarchCacheImpl) {
              CacheTransactionManager txMgr = ((MonarchCacheImpl) cache).getTxManager();
              if (txMgr != null) {
                if (txMgr.exists()) {
                  try {
