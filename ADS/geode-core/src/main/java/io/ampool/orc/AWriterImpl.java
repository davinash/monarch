...
public class AWriterImpl implements Writer, MemoryManager.Callback {

  private static final Logger LOG = LoggerFactory.getLogger(AWriterImpl.class);
  ...
  private ADataOutputStream rawWriter = null;
    ...
    switch (kind) {
      ...
      case LZO:
        try {
          ...
          if (loader == null) {
            loader = AWriterImpl.class.getClassLoader();
          }
          @SuppressWarnings("unchecked")
          Class<? extends CompressionCodec> lzo = (Class<? extends CompressionCodec>) loader
              .loadClass("org.apache.hadoop.hive.ql.io.orc.LzoCodec");
  ...
  /**
   * This class is used to hold the contents of streams as they are buffered. The TreeWriters write
   * to the outStream and the codec compresses the data as buffers fill up and stores them in the
   * output list. When the stripe is being written, the whole stream is written to the file.
   */
  private class BufferedStream implements OutStream.OutputReceiver {
    ...
    /**
     * Get the number of bytes that will be written to the output. Assumes the stream has already
     * been flushed.
     * 
  ...
  private class DirectStream implements OutStream.OutputReceiver {
    ...
    DirectStream(DataOutputStream output) {
      this.output = output;
    }
  ...
  /**
   * The parent class of all of the writers for each column. Each column is written by an instance
   * of this class. The compound types (struct, list, map, and union) have children tree writers
   * that write the children types.
   */
  private abstract static class TreeWriter {
     ...
     *
     * This default method is used for all types except structs, which are the typical case.
     * VectorizedRowBatch assumes the top level object is a struct, so we use the first column for
     * all other types.
     * 
     ...
     * 
     * @param builder the stripe footer that contains the information about the layout of the
     *        stripe. The TreeWriter is required to update the footer with its information.
     * @param requiredIndexEntries the number of index entries that are required. this is to check
     *        to make sure the row index is well formed.
     * @throws IOException
     */
    void writeStripe(OrcProto.StripeFooter.Builder builder, int requiredIndexEntries)
        throws IOException {
      if (isPresent != null) {
        isPresent.flush();
    ...
    OrcProto.ColumnEncoding getEncoding() {
      return OrcProto.ColumnEncoding.newBuilder().setKind(OrcProto.ColumnEncoding.Kind.DIRECT)
          .build();
    }

    /**
     * Create a row index entry with the previous location and the current index statistics. Also
     * merges the index statistics into the file statistics before they are cleared. Finally, it
     * records the start of the next index and ensures all of the children columns also create an
     * entry.
     * 
     * @throws IOException
     */
  ...
  private static abstract class StringBaseTreeWriter extends TreeWriter {
      ...
      if (useDictionaryEncoding) {
        if (isDirectV2) {
          return OrcProto.ColumnEncoding.newBuilder()
              .setKind(OrcProto.ColumnEncoding.Kind.DICTIONARY_V2)
    ...
    /**
     * This method doesn't call the super method, because unlike most of the other TreeWriters, this
     * one can't record the position in the streams until the stripe is being flushed. Therefore it
     * saves all of the entries and augments them with the final information as the stripe is
     * written.
     * 
     * @throws IOException
     */
      ...
      if (vector.isRepeating) {
        if (vector.noNulls || !vector.isNull[0]) {
          if (useDictionaryEncoding) {
            ...
            for (int i = 0; i < length; ++i) {
              directStreamOutput.write(vec.vector[0], vec.start[0], vec.length[0]);
              directLengthOutput.write(vec.length[0]);
            }
          }
          indexStatistics.updateString(vec.vector[0], vec.start[0], vec.length[0], length);
          if (createBloomFilter) {
        ...
        for (int i = 0; i < length; ++i) {
          if (vec.noNulls || !vec.isNull[i + offset]) {
            if (useDictionaryEncoding) {
              rows.add(dictionary.add(vec.vector[offset + i], vec.start[offset + i],
                  vec.length[offset + i]));
            } else {
              directStreamOutput.write(vec.vector[offset + i], vec.start[offset + i],
                  vec.length[offset + i]);
              directLengthOutput.write(vec.length[offset + i]);
            }
            indexStatistics.updateString(vec.vector[offset + i], vec.start[offset + i],
        ...
        for (int i = 0; i < length; ++i) {
          if (vec.noNulls || !vec.isNull[i + offset]) {
            ...
            if (vec.length[offset + i] >= itemLength) {
              ...
              ptr = padding;
              ptrOffset = 0;
              System.arraycopy(vec.vector[offset + i], vec.start[offset + i], ptr, 0,
                  vec.length[offset + i]);
  ...
  private static class VarcharTreeWriter extends StringBaseTreeWriter {
    ...
    VarcharTreeWriter(int columnId, TypeDescription schema, StreamFactory writer, boolean nullable)
        throws IOException {
      super(columnId, schema, writer, nullable);
      ...
      if (vector.isRepeating) {
        ...
        for (int i = 0; i < length; ++i) {
          if (vec.noNulls || !vec.isNull[i + offset]) {
            ...
            indexStatistics.updateString(vec.vector[offset + i], vec.start[offset + i], itemLength,
                1);
            if (createBloomFilter) {
  ...
  private static class DecimalTreeWriter extends TreeWriter {
    ...
    DecimalTreeWriter(int columnId, TypeDescription schema, StreamFactory writer, boolean nullable)
        throws IOException {
      super(columnId, schema, writer, nullable);
      this.isDirectV2 = isNewWriteFormat(writer);
      valueStream = writer.createStream(id, OrcProto.Stream.Kind.DATA);
      this.scaleStream = createIntegerWriter(
          writer.createStream(id, OrcProto.Stream.Kind.SECONDARY), true, isDirectV2, writer);
      recordPosition(rowIndexPosition);
    }
  ...
  private static class StructTreeWriter extends TreeWriter {
    ...
    private static void writeFields(StructColumnVector vector, TreeWriter[] childrenWriters,
  ...
  private static class ListTreeWriter extends TreeWriter {
    ...
    ListTreeWriter(int columnId, TypeDescription schema, StreamFactory writer, boolean nullable)
        throws IOException {
      super(columnId, schema, writer, nullable);
      this.isDirectV2 = isNewWriteFormat(writer);
      childrenWriters = new TreeWriter[1];
      childrenWriters[0] = createTreeWriter(schema.getChildren().get(0), writer, true);
        ...
        for (int i = 0; i < length; ++i) {
          if (!vec.isNull[i + offset]) {
            ...
            if (currentLength == 0) {
              ...
              childrenWriters[0].writeBatch(vec.keys, currentOffset, currentLength);
              childrenWriters[1].writeBatch(vec.values, currentOffset, currentLength);
              currentOffset = nextOffset;
              currentLength = nextLength;
            } else {
              currentLength += nextLength;
  ...
  private void flushStripe() throws IOException {
    ...
    if (rowsInStripe != 0) {
      ...
      int requiredIndexEntries =
          rowIndexStride == 0 ? 0 : (int) ((rowsInStripe + rowIndexStride - 1) / rowIndexStride);
  ...
  private void writeFileStatistics(OrcProto.Footer.Builder builder, TreeWriter writer)
      throws IOException {
  ...
  public void appendStripe(byte[] stripe, int offset, int length, StripeInformation stripeInfo,
    ...
    OrcProto.StripeInformation dirEntry =
        OrcProto.StripeInformation.newBuilder().setOffset(start).setNumberOfRows(rowsInStripe)
            .setIndexLength(stripeInfo.getIndexLength()).setDataLength(stripeInfo.getDataLength())
            .setFooterLength(stripeInfo.getFooterLength()).build();
    stripes.add(dirEntry);
