...
public class ClientPartitionAdvisor {
  ...
  public ClientPartitionAdvisor(int totalNumBuckets, String colocatedWith,
      ...
      final byte[] startRangeKey, final byte[] stopRangeKey, final PartitionResolver resolver) {
    ...
    try {
      if (partitionResolverName != null) {
        ...
        if (this.partitionResolver instanceof RowTuplePartitionResolver) {
          ((RowTuplePartitionResolver) this.partitionResolver)
              .setTotalNumberOfBuckets(this.totalNumBuckets);
          // Mtable name is region display name without any slashes, so removing it
          // String regionName = regionFullPath.split("\\/")[1];
          // MTableDescriptor tableDescriptor = MTableUtils.getMTableDescriptor(regionName, true);
          ((RowTuplePartitionResolver) this.partitionResolver).setStartStopRangeKey(startRangeKey,
              stopRangeKey);
        }
