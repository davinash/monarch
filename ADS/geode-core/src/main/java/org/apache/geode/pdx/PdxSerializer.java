 ...
 * {@link CacheFactory#setPdxSerializer(PdxSerializer) setPdxSerializer} or
 * {@link ClientCacheFactory#setPdxSerializer(PdxSerializer) client setPdxSerializer}. It can also
 * be configured in <code>cache.xml</code> using the <code>pdx-serializer</code> element. The same
 * PdxSerializer should be configured on each member of a distributed system that can serialize or
 * deserialize PDX data.
 * <p>
