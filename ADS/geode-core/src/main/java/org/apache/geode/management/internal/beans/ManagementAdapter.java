...
public class ManagementAdapter {
  ...
  public <K, V> void handleRegionCreation(Region<K, V> region) throws ManagementException {
    ...
    synchronized (regionOpLock) {
      ...
      // Allow the pick up of all regions so that the beans get populated even for tables
  ...
  public void handleTierCreation(TierStore tier) {

    TierStoreMBeanBridge bridge = new TierStoreMBeanBridge(tier);
    TierStoreMXBean diskStoreMBean = new TierStoreMBean(bridge);

    ObjectName diskStoreMBeanName = MBeanJMXAdapter.getTierStoreMBeanName(
        cacheImpl.getDistributedSystem().getDistributedMember(), tier.getName());
    ObjectName changedMBeanName = service.registerInternalMBean(diskStoreMBean, diskStoreMBeanName);

    service.federate(changedMBeanName, TierStoreMXBean.class, true);

    Notification notification = new Notification(JMXNotificationType.TIER_STORE_CREATED,
        memberSource, SequenceNumber.next(), System.currentTimeMillis(),
        ManagementConstants.TIER_STORE_CREATED_PREFIX + tier.getName());
    memberLevelNotifEmitter.sendNotification(notification);
    memberMBeanBridge.addTierStore(tier);

  }

  /* AMPOOL RELATED CHANGES END */
  /**
