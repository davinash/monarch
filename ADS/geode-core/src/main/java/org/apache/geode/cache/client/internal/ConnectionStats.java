...
public class ConnectionStats implements MessageStats {
  ...
  static {
    try {
      ...
      type = f.createType("ClientStats", "Statistics about client to server communication",
          new StatisticDescriptor[] {
              ...
              f.createLongCounter("addPdxTypeTime",
                  ...
                  "nanoseconds"),
              f.createIntGauge("mscansInProgress", "Current number of mscans being executed",
                  "mscans"),
              f.createIntCounter("mscans", "Total number of mscans completed successfully",
                  "mscans"),
              f.createIntCounter("mscanFailures", "Total number of mscan attempts that have failed",
                  "mscans"),
              f.createIntCounter("mscanTimeouts",
                  "Total number of mscan attempts that have timed out", "mscans"),
              f.createLongCounter("mscanTime",
                  "Total amount of time, in nanoseconds spent doing mscans", "nanoseconds"),});
      ...
      sendType = f.createType("ClientSendStats", "Statistics about client to server communication",
          new StatisticDescriptor[] {
              ...
              f.createLongCounter("addPdxTypeSendTime",
                  ...
                  "nanoseconds"),
              f.createIntGauge("mscanSendsInProgress",
                  "Current number of mscan sends being executed", "sends"),
              f.createIntCounter("mscanSends",
                  "Total number of mscan sends that have completed successfully", "sends"),
              f.createIntCounter("mscanSendFailures",
                  "Total number of mscan sends that have failed", "sends"),
              f.createLongCounter("mscanSendTime",
                  "Total amount of time, in nanoseconds spent doing mscan sends", "nanoseconds")});
      ///////////////////////////////////////////////////////////////////////
      /*
      ...
      mscanInProgressId = type.nameToId("mscansInProgress");
      mscanSendInProgressId = sendType.nameToId("mscanSendsInProgress");
      mscanSendFailedId = sendType.nameToId("mscanSendFailures");
      mscanSendId = sendType.nameToId("mscanSends");
      mscanSendDurationId = sendType.nameToId("mscanSendTime");
      mscanTimedOutId = type.nameToId("mscanTimeouts");
      mscanFailedId = type.nameToId("mscanFailures");
      mscanId = type.nameToId("mscans");
      mscanDurationId = type.nameToId("mscanTime");
  ...
  private final static int mscanInProgressId;
  private final static int mscanSendInProgressId;
  private final static int mscanSendFailedId;
  private final static int mscanSendId;
  private final static int mscanSendDurationId;
  private final static int mscanTimedOutId;
  private final static int mscanFailedId;
  private final static int mscanId;
  private final static int mscanDurationId;

  public long startScan() {
    this.stats.incInt(mscanInProgressId, 1);
    this.sendStats.incInt(mscanSendInProgressId, 1);
    startClientOp();
    return getStatTime();
  }

  public void endScanSend(long startTime, boolean failed) {
    long duration = getStatTime() - startTime;
    endClientOpSend(duration, failed);
    this.sendStats.incInt(mscanSendInProgressId, -1);
    int endScanSendId;
    if (failed) {
      endScanSendId = mscanSendFailedId;
    } else {
      endScanSendId = mscanSendId;
    }
    this.sendStats.incInt(endScanSendId, 1);
    this.stats.incLong(mscanSendDurationId, duration);
  }

  public void endScan(long startTime, boolean timedOut, boolean failed) {
    long duration = getStatTime() - startTime;
    endClientOp(duration, timedOut, failed);
    this.stats.incInt(mscanInProgressId, -1);
    int endScanId;
    if (timedOut) {
      endScanId = mscanTimedOutId;
    } else if (failed) {
      endScanId = mscanFailedId;
    } else {
      endScanId = mscanId;
    }
    this.stats.incInt(endScanId, 1);
    this.stats.incLong(mscanDurationId, duration);
  }
}
