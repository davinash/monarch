...
public class PartitionedRegion extends LocalRegion
  ...
  private ResultCollector executeOnMultipleNodes(final Function function,
    ...
    if (localKeys != null) {
      final RegionFunctionContextImpl prContext = new RegionFunctionContextImpl(function.getId(),
          PartitionedRegion.this, execution.getArgumentsForMember(getMyId().getId()), localKeys,
    ...
    if (targetNode.equals(localVm)) {
      ...
      final FunctionContext context =
          new RegionFunctionContextImpl(function.getId(), PartitionedRegion.this,
              execution.getArgumentsForMember(localVm.getId()), routingKeys,
              ColocationHelper.constructAndGetAllColocatedLocalDataSet(PartitionedRegion.this,
                  buckets),
              buckets, resultSender, execution.isReExecute(),
              AmpoolTableRegionAttributes.getRegionDataOrder(this.getCustomAttributes()),
              execution.getCoProcessorContext());
  ...
  public ResultCollector executeOnBucketSet(final Function function,
    ...
    if (isSelf && this.dataStore != null) {
      final RegionFunctionContextImpl prContext = new RegionFunctionContextImpl(function.getId(),
          PartitionedRegion.this, execution.getArgumentsForMember(getMyId().getId()), null,
          ColocationHelper.constructAndGetAllColocatedLocalDataSet(PartitionedRegion.this,
              localBucketSet),
          localBucketSet, resultSender, execution.isReExecute(),
          AmpoolTableRegionAttributes.getRegionDataOrder(this.getCustomAttributes()), 0
      ...
      try {
        if (isLocal) {
          ...
          if (key instanceof MTableKey) {
            if (obj != null) {
              MTableDescriptor mTableDescriptor = (MTableDescriptor) MTableUtils
                  .getTableDescriptor((MonarchCacheImpl) this.getCache(), this.getDisplayName());

              StorageFormatter storageFormatter = MTableUtils.getStorageFormatter(mTableDescriptor);

              obj = storageFormatter.performGetOperation((MTableKey) key, mTableDescriptor, obj,
                  aCallbackArgument);
  ...
  public void fillInProfile(Profile p) {
    ...
    profile.regionMapFactory = getRegionMapFactory();
    profile.customRegionAttributes = customAttributes;
  }
  ...
  protected void postDestroyRegion(boolean destroyDiskRegion, RegionEventImpl event) {
    ...
    try {
      if (AmpoolTableRegionAttributes.isAmpoolFTable(this.getCustomAttributes())) {
        MCache anyInstance = MonarchCacheImpl.getInstance();
        if (anyInstance != null && !anyInstance.isClosed()) {
          logger.info("PartitionedRegion: deleting table= {}", this.getDisplayName());
          anyInstance.getStoreHandler().deleteTable(this.getDisplayName());
        }
      }
    } catch (Throwable ex) {
      logger.warn("Store deletion failed");
    }
