...
public class LauncherLifecycleCommands extends AbstractCommandsSupport {
  ...
  protected static final String GEODE_JAR_PATHNAME =
      IOUtils.appendToPath(GEODE_HOME, "lib", GemFireVersion.getAmpoolJarFileName());

  protected static final String CORE_DEPENDENCIES_JAR_PATHNAME =
      IOUtils.appendToPath(GEODE_HOME, "lib", "monarch-dependencies.jar");

  protected static final String CORE_DEPENDENCIES_LIB_PATH =
      IOUtils.appendToPath(GEODE_HOME, "lib");

  /**
   * All extensions should go here
   */
  protected static final String MONARCH_EXTENSIONS = IOUtils.appendToPath(GEODE_HOME, "extensions");
  ...
  public Result startLocator(
      ...
      @CliOption(key = CliStrings.START_SERVER__EXT__CLASSPATH,
          /* optionContext = ConverterHint.FILE_PATHSTRING, // there's an issue with TAB here */
          unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
          help = CliStrings.START_SERVER__EXT__CLASSPATH__HELP) final String extClasspath) {
    try {
      ...
      String[] locatorCommandLine = createStartLocatorCommandLine(locatorLauncher,
          ...
          classpath, includeSystemClasspath, jvmArgsOpts, initialHeap, maxHeap, extClasspath);
  ...
  protected String[] createStartLocatorCommandLine(final LocatorLauncher launcher,
      ...
      final String maxHeap, String extClasspath) throws MalformedObjectNameException {
  ...
  protected String getLocatorClasspath(final boolean includeSystemClasspath,
      final String userClasspath, final String extClasspath) {
    List<String> jarFilePathnames = new ArrayList<>();

    jarFilePathnames.add(CORE_DEPENDENCIES_JAR_PATHNAME);

    if (extClasspath == null) {
      String[] filePaths = IOUtils.getFilePaths(CORE_DEPENDENCIES_LIB_PATH, ".jar");
      Collections.addAll(jarFilePathnames, filePaths);

      String[] filePaths1 = IOUtils.getFilePaths(MONARCH_EXTENSIONS, ".jar");
      Collections.addAll(jarFilePathnames, filePaths1);
    } else {
      jarFilePathnames.add(extClasspath);
    }
  ...
  public Result startServer(
      ...
      @CliOption(key = MashCliStrings.START_AMPOOL_SERVER__PROPERTIES,
          optionContext = ConverterHint.FILE_PATHSTRING,
          unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
          help = MashCliStrings.START_AMPOOL_SERVER__PROPERTIES__HELP) String ampoolPropertiesPathname,
      ...
      @CliOption(key = CliStrings.START_SERVER__EXT__CLASSPATH,
          ...
          unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
          help = CliStrings.START_SERVER__EXT__CLASSPATH__HELP) final String extClasspath,
      @CliOption(key = CliStrings.START_SERVER__TIER__CLASSPATH,
          /* optionContext = ConverterHint.FILE_PATHSTRING, // there's an issue with TAB here */
          unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
          help = CliStrings.START_SERVER__TIER__CLASSPATH__HELP) final String tierClasspath)
  // NOTICE: keep the parameters in alphabetical order based on their CliStrings.START_SERVER_* text
  {
    try {
      ...
      ampoolPropertiesPathname = CliUtil.resolvePathname(ampoolPropertiesPathname);

      if (!StringUtils.isBlank(ampoolPropertiesPathname)
          && !IOUtils.isExistingPathname(ampoolPropertiesPathname)) {
        return ResultBuilder.createUserErrorResult(CliStrings.format(
            MashCliStrings.AMPOOL_0_PROPERTIES_1_NOT_FOUND_MESSAGE, ampoolPropertiesPathname));
      }
      ...
      String[] serverCommandLine = createStartServerCommandLine(serverLauncher,
          gemfirePropertiesPathname, gemfireSecurityPropertiesPathname, ampoolPropertiesPathname,
          gemfireProperties, classpath, includeSystemClasspath, jvmArgsOpts,
          disableExitWhenOutOfMemory, initialHeap, maxHeap, extClasspath == null
              ? tierClasspath != null ? tierClasspath : extClasspath : extClasspath);
  ...
  protected String[] createStartServerCommandLine(final ServerLauncher launcher,
      final String gemfirePropertiesPathname, final String gemfireSecurityPropertiesPathname,
      final String ampoolPropertiesPathname, final Properties gemfireProperties,
      String userClasspath, final Boolean includeSystemClasspath, final String[] jvmArgsOpts,
      final Boolean disableExitWhenOutOfMemory, final String initialHeap, final String maxHeap,
      final String extClasspath) throws MalformedObjectNameException {
    List<String> commandLine = new ArrayList<>();
    ...
    if (ampoolPropertiesPathname != null) {
      addAmpoolOptions(commandLine, ampoolPropertiesPathname);
    }
  ...
  protected void addAmpoolOptions(List<String> commandLine, String ampoolPropertiesPathname) {
    if (ampoolPropertiesPathname == null || ampoolPropertiesPathname.isEmpty()) {
      return;
    }
    String line;
    try (BufferedReader br = new BufferedReader(new FileReader(ampoolPropertiesPathname))) {
