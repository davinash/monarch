...
public class FunctionServiceManager {
  ...
  public final Execution onMTable(MTable table) {
    if (table == null) {
      throw new FunctionException(
          LocalizedStrings.FunctionService_0_PASSED_IS_NULL.toLocalizedString("Table instance "));
    }
    Region region = ((ProxyMTableRegion) table).getTableRegion();
    if (region == null) {
      throw new FunctionException(
          LocalizedStrings.FunctionService_0_PASSED_IS_NULL.toLocalizedString("Region instance "));
    }
    ...
    if (isClientRegion(region)) {
      return new ServerRegionFunctionExecutor(table, proxyCache);
    }
    if (PartitionRegionHelper.isPartitionedRegion(region)) {
      return new PartitionedRegionFunctionExecutor(table);
    }
    return new DistributedRegionFunctionExecutor(table);
  }

  public final void registerFunction(String id, Function function) {
    if (function == null) {
      throw new FunctionException(LocalizedStrings.FunctionService_0_PASSED_IS_NULL
          .toLocalizedString("function instance "));
    }
    ...
    idToFunctionMap.put(id, function);
  }
