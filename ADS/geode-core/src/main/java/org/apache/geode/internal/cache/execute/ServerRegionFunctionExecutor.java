...
public class ServerRegionFunctionExecutor extends AbstractExecution {
  ...
  private ServerRegionFunctionExecutor(ServerRegionFunctionExecutor executor, int bucketId,
    ...
    super(executor);
    this.region = executor.region;
    this.targetBucketId = bucketId;
    this.tableExecution = executor.tableExecution;
    this.table = executor.table;
  }

  private ServerRegionFunctionExecutor(ServerRegionFunctionExecutor executor, int dummy,
      int dummy2) {
    super(executor);
    ...
    this.args = executor.args;
    this.executeOnBucketSet = executor.executeOnBucketSet;
    this.table = executor.table;
    this.tableExecution = executor.tableExecution;
    this.coProcessorContext = executor.coProcessorContext;
    this.targetBucketId = executor.targetBucketId;

  }

  public ServerRegionFunctionExecutor(MTable table, ProxyCache proxyCache) {
  ...
  @Override
  public Execution withTableSplitId(int tableSplitId) {
    if (tableSplitId < 0) {
      throw new FunctionException(
          LocalizedStrings.ExecuteRegionFunction_THE_INPUT_0_FOR_THE_EXECUTE_FUNCTION_REQUEST_IS_OUT_OF_RANGE
              .toLocalizedString("TableSplitId"));
    }
    this.targetBucketId = tableSplitId;
    return new ServerRegionFunctionExecutor(this, tableSplitId, true, this.getTable());
  }

  @Override
  public Execution withTableRowKey(byte[] rowKey) {
    throw new RuntimeException(this.getClass().getName());
  }

  @Override
  public Execution withinTableKeyRange(byte[] startKey, byte[] stopKey) {
    throw new RuntimeException(this.getClass().getName());
  }



  public int getCoProcessorContext() {
    return this.coProcessorContext;
  }

  public int getTargetBucketId() {
