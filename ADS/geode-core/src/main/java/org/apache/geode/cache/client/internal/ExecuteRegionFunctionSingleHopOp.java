...
public class ExecuteRegionFunctionSingleHopOp {
  ...
  static class ExecuteRegionFunctionSingleHopOpImpl extends AbstractOp {
    ...
    public ExecuteRegionFunctionSingleHopOpImpl(String region, Function function,
      ...
      super(MessageType.EXECUTE_REGION_FUNCTION_SINGLE_HOP,
          9 + serverRegionExecutor.getFilter().size() + removedNodes.size());
      ...
      int executionContext = serverRegionExecutor.getCoProcessorContext();
      getMessage().addObjPart(executionContext);
    ...
    public ExecuteRegionFunctionSingleHopOpImpl(String region, String functionId,
      ...
      getMessage().addObjPart(memberMappedArg);
      int executionContext = serverRegionExecutor.getCoProcessorContext();
      getMessage().addObjPart(executionContext); // Part 5
      getMessage().addBytesPart(new byte[] {allBuckets ? (byte) 1 : (byte) 0});
      getMessage().addIntPart(routingObjects.size());
      for (Object key : routingObjects) {
        if (allBuckets) {
  ...
  public static void executeOnBucket(ExecutablePool pool, Region region, String functionId,
      ServerRegionFunctionExecutor serverRegionExecutor, ResultCollector resultCollector,
      byte hasResult, MServerLocation servLoc, int mRetryAttempts, boolean allBuckets, boolean isHA,
      boolean optimizeForWrite, byte[] sampleKey, MTableType tableType) {
    ...
    SingleHopOperationCallable callableTask = constructAndGetExecuteFunctionTask(
        region.getFullPath(), serverRegionExecutor, servLoc, (PoolImpl) pool, functionId, hasResult,
        resultCollector, allBuckets, isHA, optimizeForWrite, sampleKey, tableType);

    reexecute = SingleHopClientExecutor.submitAllHA(new ArrayList<>(Arrays.asList(callableTask)),
        (LocalRegion) region, isHA, resultCollector, failedNodes);

    if (isDebugEnabled) {
      logger.debug("ExecuteRegionFunctionSingleHopOp#execute : single tasks with reexecute={}",
          reexecute);
    }
  ...
  private static Set<Integer> getBucketSet(String region, String functionId) {
    Set<Integer> bucketSet = new HashSet<>();
    String coprocessorName = (String) functionId;
    String classNameSubstr = coprocessorName.substring(region.length());
    ...
    if (logger.isDebugEnabled()) {
      ...
      logger.debug("ExecuteRegionFunctionSingleHopOp#getBucketSet : actualClassName={}",
          actualClassName);
      logger.debug("ExecuteRegionFunctionSingleHopOp#getBucketSet : bucketId={}", bucketId);
    }
    bucketSet.add(bucketId);
    return bucketSet;
  }

  static SingleHopOperationCallable constructAndGetExecuteFunctionTask(String region,
      ServerRegionFunctionExecutor serverRegionExecutor, MServerLocation servLoc,
      final PoolImpl pool, final String functionId, byte hasResult, ResultCollector rc,
      boolean allBucket, boolean isHA, boolean optimizeForWrite, byte[] bucketKey,
      MTableType tableType) {

    if (logger.isDebugEnabled()) {
      logger.debug("Constructing tasks for the server {}", servLoc);
    }

    ServerRegionFunctionExecutor executor = null;

    if (tableType == MTableType.UNORDERED) {
      executor = (ServerRegionFunctionExecutor) serverRegionExecutor
          .withBucketFilter(getBucketSet(region, functionId));
    } else {
      // Default Ordered Table case
      executor = (ServerRegionFunctionExecutor) serverRegionExecutor
          .withFilter(new HashSet(Arrays.asList(new MTableKey(bucketKey))));
    }

    AbstractOp op = new ExecuteRegionFunctionSingleHopOpImpl(region, functionId, executor, rc,
        hasResult, new HashSet<String>(), allBucket, isHA, optimizeForWrite);
    SingleHopOperationCallable task =
        new SingleHopOperationCallable(new ServerLocation(servLoc.getHostName(), servLoc.getPort()),
            pool, op, UserAttributes.userAttributes.get());
    return task;
  }
}
