...
public class ServerRegionProxy extends ServerProxy implements ServerRegionDataAccess {
  ...
  public void executeFunctionOnMTableSplit(ServerRegionFunctionExecutor sre, Function fn,
      ResultCollector resultCollector, byte hasResult, boolean replaying) {

    int retryAttempts = pool.getRetryAttempts();
    MTable table = sre.getTable();
    MServerLocation serverLocation = ((MTableLocationInfoImpl) table.getMTableLocationInfo())
        .getMTablePrimaryBucketLocation(sre.getTargetBucketId());
    if (serverLocation == null)
      return;
    byte[] sampleKey =
        MTableUtils.getSampleKeyForBucketId(table.getTableDescriptor(), sre.getTargetBucketId());

    ExecuteRegionFunctionSingleHopOp.executeOnBucket(this.pool, this.region, fn.getId(), sre,
        resultCollector, hasResult, serverLocation, retryAttempts, false, fn.isHA(),
        fn.optimizeForWrite(), sampleKey, table.getTableDescriptor().getTableType());
  }
