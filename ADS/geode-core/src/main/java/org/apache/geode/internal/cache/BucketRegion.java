...
public class BucketRegion extends DistributedRegion implements Bucket {
   ...
   * @param event
   */
  public boolean beginLocalWrite(EntryEventImpl event) {
   ...
   */
  public void endLocalWrite(EntryEventImpl event) {
    if (!needWriteLock(event)) {
      return;
    }
   ...
   */
  public RawValue getSerialized(Object key, boolean updateStats, boolean doNotLockEntry,
  ...
  @Override
  public int calculateValueSize(Object val, RegionEntry re, Object oldValue) {
    // Only needed by BucketRegion
    int size = calcMemSize(val);
    return size;
  }
