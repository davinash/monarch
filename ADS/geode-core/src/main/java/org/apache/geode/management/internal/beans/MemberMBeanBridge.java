...
import org.apache.geode.management.internal.beans.stats.MemberLevelTierMonitor;
...
public class MemberMBeanBridge {
  ...
  private MemberLevelTierMonitor tierMonitor;
  ...
  /**
   * Return the full-path of all Ampool tables that exist on the system. This filters out all the
   * Geode regions, indicated by the default (0) region- data-order. Also, if the tables with
   * specific region-data-order are requested return only the tables with the respective data orders
   * in sorted order.
   *
   * @return a list of Ampool table names hosted on the system
   */
  public String[] getListOfTables(final int... regionDataOrders) {
    Set<LocalRegion> listOfAppRegions = cache.getApplicationRegions();
    if (listOfAppRegions != null && listOfAppRegions.size() > 0) {
      return listOfAppRegions.stream().filter(e -> {
        int rdo =
            ((AmpoolTableRegionAttributes) e.getCustomAttributes()).getRegionDataOrder().getId();
        if (rdo == 0) {
          return false;
        } else if (regionDataOrders == null || regionDataOrders.length == 0) {
          return true;
        } else {
          for (int order : regionDataOrders) {
            if (order == rdo) {
              return true;
            }
          }
      ...
      }).map(LocalRegion::getFullPath).sorted().toArray(String[]::new);
    }
    return ManagementConstants.NO_DATA_STRING;
  }

  /**
   * Get the list of all available tier stores on the member. The returned list is unique and
   * sorted.
   *
   * @param args arguments, if any
   * @return the list of all available tier stores
   */
  public String[] getListOfTierStores(final int args) {
    MCache mCache = MCacheFactory.getAnyInstance();
    final Map<String, TierStore> stores = mCache.getStoreHandler().getTierStores();
    if (stores != null && stores.size() > 0) {
      return stores.keySet().stream().sorted().toArray(String[]::new);
    }
    return ManagementConstants.NO_DATA_STRING;
  }

  /* AMPOOL RELATED CHANGES START */
  public void addTierStore(TierStore tier) {
    TierStore impl = (TierStore) tier;
    addTierStoreStats(impl.getStats());
  }

  private void addTierStoreStats(TierStoreStats stats) {
    /*
     ...
     */
    try {
      tierMonitor.addStatisticsToMonitor(stats.getStats());
    } catch (Exception e) {
