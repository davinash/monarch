...
public class LocalRegion extends AbstractRegion implements LoaderHelperFactory,
  ...
  // of a snapshot, etc.
  protected volatile boolean reinitialized_old = false;
  ...
  protected final Object subregionsLock = new Object();
  ...
  private RegionMap createRegionMap(InternalRegionArguments internalRegionArgs) {
    ...
    if (result == null) {
      result = RegionMapFactory.getRegionMapFactory(this.getRegionMapFactory()).create(this,
          internalRegionArgs);
    }
    return result;
  }
   ...
   */
  public boolean evictDestroy(LRUEntry entry) {
  ...
  public void initializationFailed(LocalRegion subregion) {
  ...
  protected DiskStoreImpl findDiskStore(RegionAttributes ra,
    ...
    if (usesDiskStore(ra)) {
      if (getDiskStoreName() != null) {
        ...
        if (diskStore == null) {
          if (getGemFireCache().isAmpoolSystem()) {
            diskStore = (DiskStoreImpl) getGemFireCache().createDiskStoreFactory()
                .create(getDiskStoreName());
          } else {
  ...
  protected DiskRegion createDiskRegion(InternalRegionArguments internalRegionArgs)
    ...
    if (dsi != null) {
      ...
      return DiskRegion.create(dsi, getFullPath(), false, getDataPolicy().withPersistence(),
          ...
          getRegionMapFactory(), this.customAttributes);
    } else {
      return null;
    }
  }
  ...
  public int calculateValueSize(Object val, RegionEntry regionEntry, Object oldValue) {
    // Only needed by BucketRegion
    return 0;
  }
