 ...
 * @since GemFire 6.6
 */
public class GetClientPartitionAttributesCommand91 extends BaseCommand {

  private final static GetClientPartitionAttributesCommand91 singleton =
      new GetClientPartitionAttributesCommand91();

  public static Command getCommand() {
    return singleton;
  }

  GetClientPartitionAttributesCommand91() {}
  ...
  public void cmdExecute(Message msg, ServerConnection servConn, long start)
    ...
    try {
      ...
      if (!(region instanceof PartitionedRegion)) {
        ...
        bktStartStopInfo.setBucketCount(prRgion.getTotalNumberOfBuckets());
        if (AmpoolTableRegionAttributes.getRegionDataOrder(
            prRgion.getCustomAttributes()) == RegionDataOrder.ROW_TUPLE_ORDERED_VERSIONED) {
          String regionName = regionFullPath.split("\\/")[1];
          MTableDescriptor tableDescriptor =
              MCacheFactory.getAnyInstance().getMTableDescriptor(regionName);
          bktStartStopInfo.setStartRangeKey(tableDescriptor.getStartRangeKey());
          bktStartStopInfo.setStopRangeKey(tableDescriptor.getStopRangeKey());
        }
        responseMsg.addObjPart(bktStartStopInfo);
        ...
        // PART 3
        if (partitionResolver != null) {
          if (partitionResolver instanceof RowTuplePartitionResolver) {
            responseMsg.addObjPart(partitionResolver);
          } else {
