...
public abstract class AbstractRegionEntry implements RegionEntry, HashEntry<Object, Object> {
  ...
  public final boolean destroy(LocalRegion region, EntryEventImpl event, boolean inTokenMode,
    ...
    if (proceed) {
      /**
       * retrieve MOpInfo, if any, or create and set in event for accessing the same in co-processor
       * observers (pre/post).
       */
      MOpInfo opInfo = event.getOpInfo();
      if (opInfo == null && event.getKey() instanceof MTableKey) {
        opInfo = MOpInfo.fromKey((MTableKey) event.getKey());
  ...
  public Object prepareValueForCache(RegionEntryContext r,
    ...
    Object ampoolValue = RowTupleBytesUtils.prepareValueForAmpoolCache(event);
    if (ampoolValue != null) {
      val = ampoolValue;
    }
