...
public class AsyncEventQueueFactoryImpl implements AsyncEventQueueFactory {
  ...
  public AsyncEventQueueFactory addMAsyncEventListener(MAsyncEventListener listener) {
    this.attrs.addMAsyncEventListener(listener);
    return this;
  }

  public AsyncEventQueue create(String asyncQueueId, MAsyncEventListener listener) {
    if (listener == null) {
      throw new IllegalArgumentException(
          LocalizedStrings.AsyncEventQueue_ASYNC_EVENT_LISTENER_CANNOT_BE_NULL.toLocalizedString());
    }
    ...
    if (this.cache instanceof GemFireCacheImpl) {
      ...
      logger.info("AsyncEventQueueFactoryImpl create addingMAsyncEventListener");
      addMAsyncEventListener(listener);
      GatewaySender sender =
          create(AsyncEventQueueImpl.getSenderIdFromAsyncEventQueueId(asyncQueueId));
      AsyncEventQueueImpl queue = new AsyncEventQueueImpl(sender, listener);
      asyncEventQueue = queue;
