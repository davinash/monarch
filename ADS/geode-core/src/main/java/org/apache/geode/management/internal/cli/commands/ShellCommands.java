...
import io.ampool.security.KerberosAuthInit;
...
public class ShellCommands implements CommandMarker {
  ...
  public Result connect(
    ...
    try {
      ...
      if (gfSecurityPropertiesPath != null && gfSecurityPropertiesPath.length() > 0) {
        securityProperties.load(new FileInputStream(gfSecurityPropertiesPath));
      }

      if (securityProperties.getProperty(ENABLE_KERBEROS_AUTHC) != null
        ...
        securityProperties = krbAuthInit.getCredentials(securityProperties);
      }
    } catch (IOException e) {
    ...
    if (useHttp) {
      ...
      result = jmxConnect(sslConfigProps, memberRmiHostPort, locatorTcpHostPort, useSsl, userName,
          passwordToUse, gfSecurityPropertiesPath, securityProperties, false);
    }
  ...
  private Result jmxConnect(Map<String, String> sslConfigProps,
      ...
      String userName, String passwordToUse, String gfSecurityPropertiesPath,
      Properties securityProperties, boolean retry) {
    ...
    try {
      ...
      JmxOperationInvoker operationInvoker = new JmxOperationInvoker(hostPortToConnect.getHost(),
          hostPortToConnect.getPort(), userName, passwordToUse, sslConfigProps, securityProperties);
      ...
      try {
        ...
        return jmxConnect(sslConfigProps, hostPortToConnect, null, useSsl, userName, passwordToUse,
            gfSecurityPropertiesPath, securityProperties, true);
      } catch (IOException ioe) {
