...
public class CreateRegionProcessor implements ProfileExchangeProcessor {
  ...
  public static final class CreateRegionMessage extends HighPriorityDistributionMessage
    ...
    protected String checkCompatibility(CacheDistributionAdvisee rgn, CacheProfile profile) {
      ...
      final CustomRegionAttributes otherCustomAttributes =
          ((LocalRegion) rgn).getCustomAttributes();
      ...
      final String otherRegionMapFactory = rgn.getAttributes().getRegionMapFactory();

      if (!isRemoteAccessor(profile) && !thisIsRemoteAccessor
          && !StringUtils.compare(profile.regionMapFactory, otherRegionMapFactory)) {
        result =
            LocalizedStrings.CreateRegionProcessor_CANNOT_CREATE_REGION_0_WITH_REGIONMAP_FACTORY_EQUALS_1_BECAUSE_ANOTHER_CACHE_2_HAS_SAME_THE_REGION_WITH_REGIONMAP_FACTORY_EQUALS_3
                .toLocalizedString(
                    new Object[] {this.regionPath, profile.isOffHeap, myId, otherIsOffHeap});
      }

      if (!isRemoteAccessor(profile) && !thisIsRemoteAccessor && !CustomRegionAttributes
          .compare(profile.customRegionAttributes, otherCustomAttributes)) {
        result =
            LocalizedStrings.CreateRegionProcessor_CANNOT_CREATE_REGION_0_WITH_CUSTOM_ATTRIBUTES_EQUALS_1_BECAUSE_ANOTHER_CACHE_2_HAS_SAME_THE_REGION_WITH_CUSTOM_ATTRIBUTES_EQUALS_3
                .toLocalizedString(new Object[] {this.regionPath, profile.customRegionAttributes,
                    myId, otherCustomAttributes});
      }
