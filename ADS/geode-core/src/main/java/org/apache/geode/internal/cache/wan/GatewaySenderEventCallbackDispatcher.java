...
import io.ampool.monarch.table.internal.CDCEventImpl;
...
public class GatewaySenderEventCallbackDispatcher implements GatewaySenderEventDispatcher {
  ...
  private volatile List<MAsyncEventListener> meventListeners = Collections.emptyList();
  ...
  public void registerMAsyncEventListener(MAsyncEventListener listener) {
    synchronized (eventLock) {
      List<MAsyncEventListener> oldListeners = this.meventListeners;
      if (!oldListeners.contains(listener)) {
        List<MAsyncEventListener> newListeners = new ArrayList<MAsyncEventListener>(oldListeners);
        newListeners.add(listener);
        this.meventListeners = newListeners;
      }
    }
  }
  ...
  protected void initializeEventListeners() {
    ...
    for (MAsyncEventListener listener : this.eventProcessor.getSender().getMAsyncEventListeners()) {
      registerMAsyncEventListener(listener);
    }
  ...
  protected boolean dispatchBatch(List events) throws GatewaySenderException {
    ...
    try {
      ...
      List cdcEvents = new ArrayList<CDCEvent>();
      for (MAsyncEventListener listener : this.meventListeners) {
        for (int i = 0; i < events.size(); i++) {
          AsyncEvent asyncEvent = (AsyncEvent) events.get(i);
          CDCEvent cdcEvent = new CDCEventImpl(asyncEvent);
          cdcEvents.add(cdcEvent);
        }
        boolean successOne = listener.processEvents(cdcEvents);
        if (!successOne) {
          successAll = false;
        }
      }
