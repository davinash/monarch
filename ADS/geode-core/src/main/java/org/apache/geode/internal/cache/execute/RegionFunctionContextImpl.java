...
public class RegionFunctionContextImpl extends FunctionContextImpl
  ...
  private final RegionDataOrder regionDataOrder;
  private int targetedBucketId = -1;
  ...
  public RegionFunctionContextImpl(final String functionId, final Region dataSet, final Object args,
      ...
      RegionDataOrder regionDataOrder, int coProcessorContext) {
    ...
    if (coProcessorContext == 1
        && (this.regionDataOrder == RegionDataOrder.ROW_TUPLE_ORDERED_VERSIONED
            || this.regionDataOrder == RegionDataOrder.ROW_TUPLE_UNORDERED)) {
      if (localBucketSet.size() == 1) {
        this.targetedBucketId = localBucketSet.iterator().next();
      } else {
        throw new IllegalStateException("More than 1 bucket id is passed co-processor Execution");
      }
    }
    setFunctionContexts();
  }
