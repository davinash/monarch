...
public class Get70 extends BaseCommand {
  ...
  public void cmdExecute(Message msg, ServerConnection servConn, long startparam)
    ...
    try {
      // for integrated security, For ampool internal meta-region, skip the security check.
      // Using regionName.substring(1) while matching against metaRegion name, without the first "/"
      // char.
      if (!(MTableUtils.AMPL_META_REGION_NAME).equalsIgnoreCase(regionName.substring(1))) {
    ...
    if (key instanceof MTableKey) {
      performAmpoolGet(msg, servConn, region, regionName, key, callbackArg, start, stats,
          getContext);
    } else {
      performGeodeGet(msg, servConn, region, regionName, key, callbackArg, start, stats,
          getContext);
    }
  }


  private void performAmpoolGet(Message msg, ServerConnection servConn, Region region,
      String regionName, Object key, Object callbackArg, long start, CacheServerStats stats,
      GetOperationContext getContext) throws IOException {
    Object originalData = null;
    try {
      // Hook for the preGet operation
      byte[] retVal = null;
      Get get = null;
      MTableKey mKey = (MTableKey) key;
      if (mKey.isGetOp()) {
        String dummyValue = "VALUE_FILTERED_PRE_GET";
        get = MCoprocessorUtils.constructMGet(mKey.getBytes(), mKey.getColumnPositions(),
            region.getName());
        boolean byPass = RowTupleBytesUtils.invokePreGet(region.getName(), get);

        if (byPass) {
          retVal = ((MonarchCacheImpl) servConn.getCache()).getDummyValue(dummyValue.getBytes(),
              mKey.getColumnPositions(), region.getName());
        }
      }
      // Pre-get hook ends here.
      ...
      if (retVal == null) {
        ...
        try {
           ...
           * System.out.println("Get70.performAmpoolGet  --> " + Arrays.toString((byte[])
           * entry.value)); RowTupleValue rowTupleValue = new RowTupleValue(entry.value,
           * regionName.substring(1)); entry.value = rowTupleValue.performGetOperation((MTableKey)
           * key); System.out.println("Get70.performAmpoolGet.AFTER.PROCESSING  --> " +
           * Arrays.toString((byte[]) entry.value)); }
           */
        } catch (Exception e) {
          writeException(msg, e, false, servConn);
          servConn.setAsTrue(RESPONDED);
        ...
        // TODO:MERGE do we need this?
        // post process
        data = this.securityService.postProcess(regionName, key, data, entry.isObject);

        if (logger.isDebugEnabled()) {
          logger.debug("{}: Writing get response back to {} for region {} {}", servConn.getName(),
              servConn.getSocketString(), regionName, entry);
        }
        // post-auth ends here

        // Post-Get Hook.
        RowTupleBytesUtils.invokePostGet(region.getName(), get);
  ...
  private void performGeodeGet(Message msg, ServerConnection servConn, Region region,
      String regionName, Object key, Object callbackArg, long start, CacheServerStats stats,
      GetOperationContext getContext) throws IOException {
    {
  ...
  // private void performAmpoolGet(Message msg, ServerConnection servConn,
  // Region region,
  // String regionName,
  // Object key,
  // Object callbackArg,
  // long start,
  // CacheServerStats stats) throws IOException {
  //
  // GetOperationContext getContext = null;
  // try {
  ...
  //
  // if (byPass) {
  // retVal = RowTupleBytesUtils.getDummyValue(dummyValue.getBytes(), mKey.getColumnPositions(),
  // region.getName());
  // }
  // }
  //
  ...
  // }
  //
  // private void performGeodeGet(Message msg, ServerConnection servConn,
  // Region region,
  // String regionName,
  // Object key,
  // Object callbackArg,
   ...
   */
  @Retained
  public Entry getEntry(Region region, Object key, Object callbackArg, ServerConnection servConn) {
