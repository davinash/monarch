...
public class InternalDistributedSystem extends DistributedSystem
  ...
  private void reconnect(boolean forcedDisconnect, String reason) {
    ...
    boolean inhibitCacheForAmpool = false;
    if (cache != null) {
      if (cache.isAmpoolSystem()) {
        inhibitCacheForAmpool = true;
      }
    ...
    try {
      while (this.reconnectDS == null || !this.reconnectDS.isConnected()) {
        ...
        try {
          ...
          if (inhibitCacheForAmpool) {
            final MonarchCacheImpl mCache = MonarchCacheImpl.getInstance();
            if (mCache != null) {
              mCache.closeAmpoolCache();
            }
          }
        } catch (Exception ee) {
          ee.printStackTrace();
        ...
        if (newDM instanceof DistributionManager) {
          ...
          if (((DistributionManager) newDM).getDMType() != DistributionManager.ADMIN_ONLY_DM_TYPE) {
            try {
              ...
              if (inhibitCacheForAmpool) {
                cache = (GemFireCacheImpl) MonarchCacheImpl.create(this.reconnectDS, config);
              } else {
