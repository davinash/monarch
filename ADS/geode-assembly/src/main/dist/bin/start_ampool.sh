...
help(){
    echo "---------------------------------------------------------------------------------"
    echo "1. To start locator:"
    echo "   start_ampool.sh -s locator -n <locator_name> [-p <locator_port>]"
    echo "2. To start server:"
    echo "   start_ampool.sh -s server -n <server_name> -l <locator_host[port]> [-p <server_port>] [-c <jars to add into the classpath>] [-x <max heap size. e.g. 10g>] [-b <socket buffer size>]"
    echo "3. Help:"
    echo "   start_ampool.sh -h"
    echo "---------------------------------------------------------------------------------"
    exit 1
}
...
while getopts :s:n:p:l:c:x:b:h opt;
do
    case $opt in
        s) # Get service
          SERVICE="$OPTARG"
	  if [ "$SERVICE" = "locator" ]; then
		SERVICE_PORT=$LOCATOR_PORT
	  fi
          isService=true
          ;;
        n) # Get service name
...
if [ "$SERVICE" = "locator" ]; then
    ...
    if [ "$OPERATION" = "start" ]; then
        ...
        # check if classpath is provided
        start_cmd="start server --name=$SERVICE_NAME  --server-port=$SERVICE_PORT --locators=$LOCATOR_ADD"
        if [ "$isClasspath" = "true" ]; then
            start_cmd="$start_cmd --classpath=$CLASSPATH"
        fi
        if [ "$isMaxHeap" = "true" ]; then
            start_cmd="$start_cmd --max-heap=$HEAP"
        fi
        if [ "$isSockBuffSize" = "true" ]; then
            start_cmd="$start_cmd --socket-buffer-size=$SOCK_BUFF_SIZE"
        fi
        $GEODE_HOME/mash -e "$start_cmd"
        # End of start server
    elif [ "$OPERATION" = "stop" ]; then
