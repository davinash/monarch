 ...
 * example uses the kerberos authentication which is supported by the Ampool cluster. Before running
 * this example please refer to instructions on setting up secured ampool cluster with kerberos
 * authentication. Also specify the kerberos properties in the example below.
 */
public class SecuredAmpoolClientWithKerberosAuthentication {
  private static final String[] COLUMN_NAMES =
      new String[] {"NAME", "ID", "AGE", "SALARY", "DEPT", "DOJ"};

  public static void main(String args[]) {
    System.out.println("Running SecuredAmpoolClientWithKerberosAuthentication example!");
    ...
    props.setProperty(Constants.MClientCacheconfig.MONARCH_CLIENT_LOG,
        "/tmp/SecuredAmpoolClientWithKerberosAuthentication.log");
    props.setProperty(DistributionConfig.SECURITY_CLIENT_AUTH_INIT_NAME,
        "io.ampool.security.AmpoolAuthInitClient.create");
    props.setProperty(KERBEROS_SERVICE_PRINCIPAL, "<server principal>");
    props.setProperty(KERBEROS_SECURITY_USERNAME, "<user principal>");
    props.setProperty(KERBEROS_SECURITY_PASSWORD, "<user keytab>");
