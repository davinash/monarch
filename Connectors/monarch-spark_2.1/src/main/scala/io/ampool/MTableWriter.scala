    ...
    dataFrame.foreachPartition { rdd =>
      ...
      rdd.zipWithIndex.grouped(batchSize).foreach { chunk =>
        val puts = chunk.map { pair =>
          ...
          pair._1.toSeq.zipWithIndex.foreach{ case (v, i) =>
            put.addColumn(columns(i), DataConverter.convertWrite(schema(i).dataType, v))
          }
    ...
    dataFrame.foreachPartition { rdd =>
      ...
      rdd.zipWithIndex.grouped(batchSize).foreach { chunk =>
        val puts = chunk.map { pair =>
          val put = putList(pair._2 % batchSize)
          pair._1.toSeq.zipWithIndex.foreach{ case (v, i) =>
            put.add(columns(i), DataConverter.convertWrite(schema(i).dataType, v))
          }
          recCount += 1
          put
        }.toArray
