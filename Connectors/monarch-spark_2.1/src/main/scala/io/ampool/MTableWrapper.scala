...
class MTableWrapper(var tableName: String = null,
  ...
  def createAndGet(schema: StructType, overwrite: Boolean): InternalTable = {
    ...
    if (tableExists) {
      ...
      if("immutable".equalsIgnoreCase(tableType)){
        ...
        var str = parameters.getOrElse(Constants.AmpoolBlockSize, null)
        try {
          if (str != null) td.setBlockSize(str.toInt)
        } catch {
          case _: Throwable => Logger.getLogger(classOf[MTableWriter])
            .error(s"Invalid block-size= $str")
        }
        str = parameters.getOrElse(Constants.AmpoolBlockFormat, null)
        try {
          if (str != null) td.setBlockFormat(FTableDescriptor.BlockFormat.valueOf(str))
        } catch {
          case _: Throwable => Logger.getLogger(classOf[MTableWriter])
            .error(s"Invalid block-format= $str")
        }
  ...
  def createAndGetMTable(schema: StructType, tableType: String): InternalTable = {
    ...
    // setting max-versions for ordered table
