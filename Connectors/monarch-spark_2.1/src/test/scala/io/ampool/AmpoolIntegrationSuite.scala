...
class AmpoolIntegrationSuite extends FunSuite with BeforeAndAfterAll with Matchers {
  ...
  /** defaults **/
  val tableName: String = "test_immutable"
  ...
  test("WriteAndReadComplexTypesWithPersistenceAndRestart") {
    ...
    val results4 = TestHelper.dataFrameToStringComplexTypes(df1)
    val results5 = TestHelper.mTableToStringComplexTypes(newTableName, params)
    val results6 = TestHelper.dataFrameToStringComplexTypes(df2)
